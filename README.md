...
1. 易有云 2.0 还远未完善，目前是体验版，体验版！对于要求比较高的请等待正式版，使用前请务必看下面的帮助！
...
4. 本地双盘备份，异地双设备备份，满足尽可能的数据安全
5. 支持单终端同时管理多个存储端
6. 存储端支持多用户
7. 通信链路安全，保证通信链路加密，不同的用户使用不同的密钥
8. 通信链路智能选择（自动使用局域网链路，点对点链路，或者 CDN 节点链路）
9. 共有协议支持（目前核心要支持 SFTP、Samba、Webdav）
10. 全平台支持：iOS/Android/Windows/macOS/Android-TV
11. 开放接口（TODO）
12. 易有云社区化运营（TODO）
...
1. 必须先用 iOS 或者 Andriod 注册一个账号（目前仅支持手机号，微信不支持，电脑端也不支持注册账号）
2. 必须先安装一个存储端（请看上面的存储端下载链接进行下载）
3. 访问存储端设备的 http://ip:8897/preweb/ 可以看到二维码或者序列号，以及存储的路径（本来这里应该有个安装向导，目前暂时没实现）
4. 用手机扫码，或者用 Windows macOS 端添加设备，输入序列号（Windows 端支持自动识别局域网内的存储端设备，点自定义后等 5 秒钟会出现局域网内的未绑定的设备）
5. 终端与存储端绑定成功，存储端的第一个用户为设备管理员，进一步可以分享设备给其他用户
6. 目前电脑端功能相对好一些，手机端的 UI 跟交互将全部改版中
